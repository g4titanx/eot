...
fn dynamic_gas_calculation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Pre-warm a storage slot
    let address = vec![0x12u8; 20];
    let storage_key = 0x123u64.to_be_bytes();
    ...
    println!("  Savings from warming: {} gas ({:.1}%)", 
             cold_cost - warm_cost, 
    ...
    println!("  Memory expansion overhead: {} gas", large_memory - small_memory);
...
fn analyze_erc20_transfer() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let transfer_sequence = vec![
        ...
        (0x55, vec![0x1001]), // SSTORE - update sender balance
        (0x55, vec![0x1002]), // SSTORE - update receiver balance
        (0xa1, vec![]),       // LOG1 - Transfer event
    ];
...
fn fork_comparison_analysis() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let forks = vec![Fork::Istanbul, Fork::Berlin, Fork::London, Fork::Shanghai, Fork::Cancun];
    ...
    for change in changes.iter().take(5) {
        if let (Some(old), Some(new)) = (change.old_value, change.new_value) {
            println!("  0x{:02x}: {} → {} gas ({:+} gas)",
                     change.opcode, old, new, new as i32 - old as i32);
        }
    }
    ...
    println!("  Opcodes with gas changes: {}", report.summary.gas_cost_changes);
...
fn gas_optimization_analysis() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Efficiency score: {}", original_analysis.efficiency_score());

    println!("\nOptimized contract:");
    println!("  Total gas: {}", optimized_analysis.total_gas);
    println!("  Efficiency score: {}", optimized_analysis.efficiency_score());
...
fn bytecode_efficiency_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Savings:    {} gas", old_push_analysis.total_gas - new_push_analysis.total_gas);
    ...
    println!("  Simple ISZERO:  {} gas (efficiency: {}%)", 
             simple_analysis.total_gas, simple_analysis.efficiency_score());
    println!("  Complex check:  {} gas (efficiency: {}%)", 
             complex_analysis.total_gas, complex_analysis.efficiency_score());
...
mod tests {
    ...
    #[test]
    fn test_basic_gas_analysis() {
        // This should not panic and should return reasonable gas costs
        assert!(basic_gas_analysis().is_ok());
    }

    #[test]
    fn test_optimization_saves_gas() {
        let calculator = DynamicGasCalculator::new(Fork::London);
        
        // Original with repeated SLOAD
        let original = vec![
            (0x54, vec![0x100]),
            (0x54, vec![0x100]), 
            (0x54, vec![0x100]),
        ...
        let original_analysis = calculator.analyze_sequence_gas(&original).unwrap();
        let optimized_analysis = calculator.analyze_sequence_gas(&optimized).unwrap();

        assert!(optimized_analysis.total_gas < original_analysis.total_gas);
    }

    #[test]
    fn test_shanghai_push0_optimization() {
        let calculator = DynamicGasCalculator::new(Fork::Shanghai);
        
        let old_push = vec![(0x60, vec![0])]; // PUSH1 0x00
        let new_push = vec![(0x5f, vec![])];  // PUSH0

        let old_analysis = calculator.analyze_sequence_gas(&old_push).unwrap();
        let new_analysis = calculator.analyze_sequence_gas(&new_push).unwrap();

        // PUSH0 should be more gas efficient
        assert!(new_analysis.total_gas <= old_analysis.total_gas);
    }

    #[test]
    fn test_fork_comparison() {
        let calculator_istanbul = DynamicGasCalculator::new(Fork::Istanbul);
        let calculator_berlin = DynamicGasCalculator::new(Fork::Berlin);
        let context = ExecutionContext::new();

        // SLOAD should be more expensive in Berlin due to EIP-2929
        let istanbul_cost = calculator_istanbul.calculate_gas_cost(0x54, &context, &[0x123]).unwrap();
        let berlin_cost = calculator_berlin.calculate_gas_cost(0x54, &context, &[0x123]).unwrap();

        assert!(berlin_cost > istanbul_cost);
    }

    #[test]
    fn test_gas_analysis_efficiency_scoring() {
        let calculator = DynamicGasCalculator::new(Fork::London);
        
        // Efficient sequence
        let efficient = vec![(0x01, vec![]), (0x02, vec![])]; // ADD, MUL
        let efficient_analysis = calculator.analyze_sequence_gas(&efficient).unwrap();
        
        // Inefficient sequence with many expensive operations
        let inefficient = vec![
            (0x54, vec![0x100]),
            (0x55, vec![0x100]),
            (0x54, vec![0x200]),
        ...
        let inefficient_analysis = calculator.analyze_sequence_gas(&inefficient).unwrap();

        assert!(efficient_analysis.efficiency_score() > inefficient_analysis.efficiency_score());
    }
}
